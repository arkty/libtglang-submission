...
DOES not include exercise body post, problems, solutions or comments */
...
/* calculates text search vector for the individual exercise row */
...
BEGIN
    query = 'SELECT concat_tsvectors(text_search_vector) ' ||
       ...
       ' AND post_type=''problem'' AND deleted=false';
    FOR onerow IN EXECUTE query LOOP
        RETURN onerow.concat_tsvectors;
    END LOOP;
    RETURN to_tsvector('');
...
CREATE OR REPLACE FUNCTION get_dependent_solutions_tsv(thread_id integer)
RETURNS tsvector AS
$$
DECLARE
    query text;
...
BEGIN
    query = 'SELECT concat_tsvectors(text_search_vector) ' ||
       'FROM askbot_post WHERE thread_id = ' || thread_id ||
       ' AND post_type=''solution'' AND deleted=false';
    FOR onerow IN EXECUTE query LOOP
        RETURN onerow.concat_tsvectors;
    END LOOP;
    RETURN to_tsvector('');
...
UPDATE askbot_post SET text_search_vector = get_post_tsv(text, 'solution') WHERE post_type='solution';
...
UPDATE askbot_post as q SET text_search_vector = text_search_vector ||
    get_dependent_comments_tsv(q.id) WHERE post_type IN ('exercise', 'problem', 'solution');
...
BEGIN
    ...
    ELSIF new.post_type = 'problem' THEN
        new.text_search_vector = get_post_tsv(new.text, 'problem') ||
                                 get_dependent_comments_tsv(new.id);
    ELSIF new.post_type = 'solution' THEN
        new.text_search_vector = get_post_tsv(new.text, 'solution') ||
                                 get_dependent_comments_tsv(new.id);
